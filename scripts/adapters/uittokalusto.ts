...
const BASE_URL = 'https://www.uittokalusto.fi';
...
export const uittokalusto: Adapter = {
  name: 'Uittokalusto',
  baseUrl: BASE_URL,
  ...
  async discover(page: Page, caliber: string): Promise<DiscoveredProduct[] | null> {
    ...
    return discoverByPattern(page, {
      caliber,
      baseUrl: BASE_URL,
      categoryUrls: [`${BASE_URL}/catalogsearch/result/?q=${q}`],
  ...
  async delivery(page: Page): Promise<DeliveryRule | null> {
    return fetchDelivery(page, {
      shippingUrl: `${BASE_URL}/toimitustavat`,
    });
  },
};
